...
open BOX,">Boxplot-$ARGV[3]-$Phenotype.txt" or die $!;
...
library("ggplot2")
df<-read.delim("Boxplot-$ARGV[3]-$Phenotype.txt",sep="\t",header=F)
df\$V1<-factor(df\$V1,levels=unique(df\$V1))
pdf(file="Plot_boxplot-$ARGV[3]-$Phenotype.pdf", width=10, height=5)
